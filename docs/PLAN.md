...
- **拖拽手势与重渲染竞争**：mousedown 触发 on_front/focus 会同步重排 z 序；若窗口
  节点被替换，旧监听器持有的 pane 会变成游离节点，mousemove 样式写不上（跟随降级），
  甚至会把错误尺寸写回并误触发顶缘吸附。现已修复：WindowManager 缓存存活窗口的
  WindowFrame，并给根节点注入稳定 `frame_key`；同一窗口跨 z 序重排复用原 DOM，
  配合节点级响应式 class/style，拖拽期间 pane 始终连接。setup_drag 仍按
  「mousedown 基准值 + 指针位移」纯数学回写，不读取手势中途的节点现状。
